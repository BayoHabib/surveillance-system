...
func (c *visionServiceClient) StreamDetections(ctx context.Context, in *DetectionStreamRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[DetectionEvent], error) {
	...
	x := &grpc.GenericClientStream[DetectionStreamRequest, DetectionEvent]{ClientStream: stream}
	if err := x.ClientStream.SendMsg(in); err != nil {
		return nil, err
	}
	if err := x.ClientStream.CloseSend(); err != nil {
...
// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type VisionService_StreamDetectionsClient = grpc.ServerStreamingClient[DetectionEvent]
...
func _VisionService_StreamDetections_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(VisionServiceServer).StreamDetections(m, &grpc.GenericServerStream[DetectionStreamRequest, DetectionEvent]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type VisionService_StreamDetectionsServer = grpc.ServerStreamingServer[DetectionEvent]
...
var VisionService_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		},
		{
			...
			Handler:       _VisionService_StreamDetections_Handler,
			ServerStreams: true,
		},
	},
